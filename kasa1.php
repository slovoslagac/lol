            ...
            foreach ($allProductsRegular as $item) {
                ...
                { ?>
                    <div class="sony sony_free">
                        ...
                        <p id="product<?php echo $item->id ?>" onmousedown="articles(event, '<?php echo $item->id ?>', 15);">+15</p>
                        <p id="product<?php echo $item->id ?>" onmousedown="articles(event, '<?php echo $item->id ?>', 30);">+30</p>
                        <p id="product<?php echo $item->id ?>" onmousedown="articles(event, '<?php echo $item->id ?>', 60);">+1h</p>
                        <p>N</p>
               ...
               <?php }
                ...
                ?>
                <div class="product-round" id="product<?php echo $item->id ?>" onmousedown="articles(event, '<?php echo $item->id ?>', 1);">
...
<script>
    ...
    function articles(event, val, amount) {
        if (event.button == 0) {
            add_product(val, amount);
        } else if (event.button == 2) {
            removeArticle(val, amount);
        }
    }

    function add_product(val, amount) {
        var code = val;
        ...
        if (productsID.indexOf(code) > -1) {
            ...
            divadd.innerHTML = '<div class="bill-row" id="article' + code + '" ><input type="hidden" name="na' + code + '" id="na' + code + '" value="1"><strong id="numarticle' + code + '">' + amount + '</strong></input><strong> x ' + articlename + '</strong><span id="checkprice' + code + '">' + price + '</span><div class="plusminus"><i class="icon-plus" onclick="addArticle(' + code + ', 1)"></i><i class="icon-minus" onclick="removeArticle(' + code + ', 1)"></i></div></div>';
